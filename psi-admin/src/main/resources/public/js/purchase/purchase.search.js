layui.use(['laydate', 'table', 'layer'], function () {
    ...
    var tableIns = table.render({
        ...
        cols: [[
            {field: "id", title: '编号', fixed: "true", width: 80, hide: true},
            ...
            {field: 'checkState', title: '审核状态', minWidth: 100, align: 'center',templet:function (d){
                    if(d.checkState!="未审核" && d.checkState!="审核未通过"){
                        return "<img src=../../images/"+d.checkState+"  width='80px' height='80px'/>";
                    }else {
                        return d.checkState;
                    }
                }},
            {field: 'remarks', title: '备注', minWidth: 100, align: 'center'},
            {title: '操作', minWidth: 200, templet: '#purchaseListBar', fixed: "right", align: "center"}
        ]]
    });
    ...
    table.on("tool(purchaseList)", function (obj) {
        ...
        if (layEvent === "search") {
            ...
            opeCheckDialog(obj.data.id)
        }
    });

    // 打开审核单据页面
    function opeCheckDialog(uid) {
        var url = ctx + "/purchase/checkPage";
        var title = "进货单审核";
        if (uid) {
            url = url + "?id=" + uid;
        }
