layui.use(['table', 'layer'], function () {
    ...
    var tableIns = table.render({
        ...
        cols: [[
            ...
            {field: 'name', title: '单位名称', minWidth: 50, align: "center", sort: true},
            {title: '操作', minWidth: 150, templet: '#goodsUnitListBar', fixed: "right", align: "center"}
        ]]
    });

    // 多条件搜索
    $(".search_btn").on("click", function () {
        table.reload("goodsUnitListTable", {
            page: {
                curr: 1 //重新从第 1 页开始
            },
            where: {
                goodsUnitName: $("input[name='goodsUnitName']").val()
            }
        })
    });
    ...
    //头工具栏事件
    table.on('toolbar(goodsUnits)', function (obj) {
     ...
     * 行监听
     */
    table.on("tool(goodsUnits)", function (obj) {
    ...
    // 打开添加页面
    function openAddOrUpdateGoodsUnitDialog(uid) {
