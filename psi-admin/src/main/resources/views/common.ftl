<#assign security=JspTaglibs["http://www.springframework.org/security/tags"]/>
<link rel="icon" href="${ctx.contextPath}/images/favicon.ico">
<link rel="stylesheet" href="${ctx.contextPath}/lib/layui-v2.5.5/css/layui.css" media="all">
<link rel="stylesheet" href="${ctx.contextPath}/css/layuimini.css" media="all">
<link rel="stylesheet" href="${ctx.contextPath}/lib/font-awesome-4.7.0/css/font-awesome.min.css" media="all">
<link rel="stylesheet" href="${ctx.contextPath}/css/formSelects-v4.css" media="all">
<link rel="stylesheet" href="${ctx.contextPath}/css/public.css" media="all">
<#--新增css-->
<link rel="stylesheet" href="${ctx.contextPath}/css/button.css" media="all">
<link rel="stylesheet" href="${ctx.contextPath}/css/loader.css" media="all">
<script src="${ctx.contextPath}/lib/layui-v2.5.5/layui.js" charset="utf-8"></script>
<script src="${ctx.contextPath}/js/lay-config.js" charset="utf-8"></script>
<#--新增js在lay-module目录-->
<#--<script src="${ctx.contextPath}/js/lay-module/admin.js" charset="utf-8"></script>-->
<script type="text/javascript">
    var ctx = "${ctx.contextPath}";
</script>
